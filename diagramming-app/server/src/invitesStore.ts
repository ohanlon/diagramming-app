...
export async function createInvite(diagramId: string, invitedEmail: string, invitedBy: string | null = null, expiresAt: string | null = null) {
  const id = uuidv4();
  const token = uuidv4();
  const now = new Date().toISOString();
  const query = `INSERT INTO pending_invites(id, token, invited_email, diagram_id, invited_by, created_at, expires_at, accepted) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`;
  const values = [id, token, String(invitedEmail).toLowerCase(), diagramId, invitedBy, now, expiresAt, false];
